...
EM.MSI <- function(Y, M, p, max.iter=100, convergence=0.0001, diagonal.PR_TR=0.5, print=TRUE, plot=TRUE) 
{   
...
### cat("\nINITIALIZATION\n______________________________________________\n")
	...
	init.results 	<- try(Initialization.MSI(Y, M, p, diagonal.PR_TR))
...
#
    ...
    expectation             <- FALSE
    maximization            <- FALSE 
    number.parameters       <- M^2 + K*(M+K*p) + K^2
...
# Regression step to get the regime-specific estimates
            max.results <- try(M.step.MSI(y=Y, p=p, M=M, fp=fp, sp=sp, PR_TR=PR_TR))
...
#
	if(convergence.code <= 1){
        ...
        IC  <- get.IC(model.type="MSI",K=K,p=p,M=M,T=TT-p,max.log.likelihood=likelihood.val[length(likelihood.val)],print.results=print)
