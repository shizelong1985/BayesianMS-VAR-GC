...
#---------------------------------------------------------------------------------------------------
#  MSAH: Initial value for starting the EM algorithm, same as MSIAH
#---------------------------------------------------------------------------------------------------
  
  
Initialization.MSAH <- function(Y, M, p, diagonal.PR_TR)
{
    TT	<- dim(Y)[1] 
	K	<- dim(Y)[2] 
	...
	for(regime in 1:M){
    	...
    	### print(y_mat_segmented[,,regime])
 		...
 		TMP <- olsvarc.MSIAH(y_mat_segmented[,,regime], p=p) 
		
		A	<- as.matrix(TMP$A)
		OLS_coeff[,,regime] <-  A[1:K,]

		# ols_coeffs_resids() returns residuals given some OLS coefficients.
		TMP <-  ols_coeffs_resids.MSIAH(y=Y, p=p, A=TMP$A_orig)
		if(K==1){
