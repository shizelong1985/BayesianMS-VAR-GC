...
EM.MSIAH <- function(Y, M, p, max.iter=100, convergence=0.0001, diagonal.PR_TR=0.5, print=TRUE, plot=TRUE) 
{   
...
### cat("\nINITIALIZATION\n______________________________________________\n")
	...
	init.results 	<- try(Initialization.MSIAH(Y, M, p, diagonal.PR_TR))
...
#---------------------------------------------------------------------------------------------------
	if(convergence.code <= 1){
        ...
        IC  <- get.IC(model.type="MSIAH", K=K, p=p, M=M, T=TT-p, max.log.likelihood=likelihood.val[length(likelihood.val)], print.results=print)
