...
#---------------------------------------------------------------------------------------------------
#  MSIA: Initial value for starting the EM algorithm  
#---------------------------------------------------------------------------------------------------
  
  
Initialization.MSIA <- function(Y, M, p, diagonal.PR_TR)
{
    TT	<- dim(Y)[1] 
	K	<- dim(Y)[2] 
	...
	for(regime in 1:M){
    	...
    	### print(y_mat_segmented[,,regime])
 		...
 		TMP <- olsvarc.MSIA(y_mat_segmented[,,regime], p=p) 
		
		A	<- as.matrix(TMP$A)
		OLS_coeff[,,regime] <-  A[1:K,]
		### cat("OLS_coeff[,,regime]", regime, "\n")
		### print(TMP$A_orig)
 
		# ols_coeffs_resids() returns residuals given some OLS coefficients.
		TMP <-  ols_coeffs_resids.MSIA(y=Y, p=p, A=TMP$A_orig)
		if(K==1){
...
ols_coeffs_resids.MSIA <- function(y, p, A)
{
	y	<- as.matrix(y)
