...
TPV.EM.MSIH <- function(Y, M, p, TPV.PR_TR, TPV.Sigma, TPV.U, TPV.Xi, max.iter=1000, convergence=0.000001) 
{   
...
#
	if(convergence.code <= 1){
        ...
        IC  <- get.IC(model.type="MSIH",K=K,p=p,M=M,T=TT-p,max.log.likelihood=likelihood.val[length(likelihood.val)],print.results=FALSE)
