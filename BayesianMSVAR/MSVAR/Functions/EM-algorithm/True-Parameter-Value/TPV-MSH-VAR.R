...
TPV.EM.MSH <- function(Y, M, p, TPV.PR_TR, TPV.Sigma, TPV.U, TPV.Xi, max.iter=1000, convergence=0.000001) 
{   
...
# Regression step to get the regime-specific estimates
            max.results <- try(M.step.MSH(y=Y, p=p, M=M, Sigma=Sigma, fp=fp, sp=sp, PR_TR=PR_TR))
            # Error handling
...
#
	if(convergence.code <= 1){
        ...
        IC  <- get.IC(model.type="MSH",K=K,p=p,M=M,T=TT-p,max.log.likelihood=likelihood.val[length(likelihood.val)],print.results=FALSE)
