...
MSAH.Parameters.Generation <- function(K, p, M, TT, regime.multiplier=1,A.max.iter=10000, A.low.max.eigen=0.5, A.up.max.eigen=0.99, A.ratio.zeros=1/2, A.max.values=1, PR.min.diag=0.9, PR.max.diag=0.95)  {
    ...
    if(out.A$return.code==1){
        ...
        return.code <- 0
        A           <- out.A$A.output
        # Same intercepts for all regimes
        for(regime in 2:M){
            A[1,,regime]    <- A[1,,1]
        }
...
MSAH.Series.Generation   <- function(K, p, M, TT, min.regime.occurrence=30, max.iter.hist=200, burnin=100, A, PR_TR, Sigma) { 
...
#---------------------------------------------------------------------------------------------------
MSAH.gen.U <- function(Sigma, p, M, TT)
{
	K <- dim(Sigma)[1]
