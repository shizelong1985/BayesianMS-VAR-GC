...
MSI.Parameters.Generation <- function(K, p, M, TT, regime.multiplier=1,A.max.iter=10000, A.low.max.eigen=0.5, A.up.max.eigen=0.99, A.ratio.zeros=1/2, A.max.values=1, PR.min.diag=0.9, PR.max.diag=0.95)  {
...
MSI.Series.Generation   <- function(K, p, M, TT, min.regime.occurrence=30, max.iter.hist=200, burnin=100, A, PR_TR, Sigma) { 
    ...
    while(condition == TRUE)    {
        ...
        regime.min	<- min(colSums(sp.from.Xi(matrix(Gen.Xi[-(1:burnin),1], ncol=1), M)))   
        ...
        if(regime.min >= min.regime.occurrence)    {
            ...
            ### cat("Gen.U \n") 
            U.tmp	   	<- MSI.gen.U(Sigma, p, M, TT+burnin)

            ### cat("Gen.Y \n")  
            Gen.Y	    <- MSI.gen.Y(A, U.tmp, Gen.Xi)$Y
            ...
            ### cat("Gen.PR_TR \n")
            Gen.PR_TR   <- count.transitions.from.Xi(Gen.Xi, M)
        }else{
...
#---------------------------------------------------------------------------------------------------
MSI.gen.U <- function(Sigma, p, M, TT)
{
	K <- dim(Sigma)[1]
