...
MSIH.Parameters.Generation <- function(K, p, M, TT, regime.multiplier=1,A.max.iter=10000, A.low.max.eigen=0.5, A.up.max.eigen=0.99, A.ratio.zeros=1/2, A.max.values=1, PR.min.diag=0.9, PR.max.diag=0.95)  {
    
    # Generate only one set of coefficients, MSIH
    ...
    if(out.A$return.code==1){
        ...
        A           <- out.A$A.output
        for(regime in 1:M){
            A[,,regime]     <- out.A$A.output[,,regime]
            A[-1,,regime]   <- A[-1,,1]
        }  
...
MSIH.Series.Generation   <- function(K, p, M, TT, min.regime.occurrence=30, max.iter.hist=200, burnin=100, A, PR_TR, Sigma) { 
    ...
    while(condition == TRUE)    {
        ...
        if(regime.min >= min.regime.occurrence)    {
            ...
            ### cat("Gen.U \n")
            U.tmp	   	<- MSIH.gen.U(Sigma, p, M, TT+burnin)
 
            ### cat("Gen.Y \n")
            Gen.Y	    <- matrix(MSIH.gen.Y(A, U.tmp, Gen.Xi)$Y, ncol=K)
...
#---------------------------------------------------------------------------------------------------
MSIH.gen.U <- function(Sigma, p, M, TT)
{
	K <- dim(Sigma)[1]
...
MSIH.gen.Y <- function(A, U, Xi)
{
	...
	for(t in 1:TT){
		regime <- Xi[t]
		Comp.Y[,t] <- as.matrix(Comp.Int[,,regime]) + as.matrix(Comp.A[,,1]) %*% as.matrix(Ym1) + as.matrix(U$Comp.U[,t,regime])
		Ym1 <- Comp.Y[,t]
	}
	Y <- t(Comp.Y)[,1:K]
