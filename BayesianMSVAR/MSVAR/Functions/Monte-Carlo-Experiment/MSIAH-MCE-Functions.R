...
MSIAH.MC.Experiment <- function(K, p, M, TT, N=1000, min.regime.occurrence, A, Sigma, PR_TR)   {
	...
	cat("\n______________________________________________________________________________\nMonte Carlo experiment, MSIAH-VAR K:", K, " p:", p, "   M:", M, "   TT:", TT, "\n______________________________________________________________________________\n") 
...
# BEGIN LOOP
        ...
        out.ser.gen     <- MSIAH.Series.Generation(K, p, M, TT, min.regime.occurrence, max.iter.hist,  burnin=100, A, PR_TR, Sigma)
        ...
        out.EM.wrapper  <- MSIAH.EM.wrapper(tmp.Gen.Y,M,p,tmp.Gen.Xi)
...
#
MSIAH.EM.wrapper  <- function(Y,M,p,Gen.Xi)   {

    Est.output  <- try(EM.MSIAH(Y, M, p, max.iter=1000, convergence=0.00001, diagonal.PR_TR=0.9, print=FALSE, plot=FALSE))
    ...
    for(regime in 1:M)  {
        ...
        Est.A[,,regime]        <- Est.output$A[,,mapping[regime]]   

        # Reorder transition probabilities
        for(column in 1:M)  {
            row.est		<- mapping[regime]
            col.est		<- mapping[column]
            Est.PR_TR[regime,column]    <-  Est.output$sp[row.est,col.est] 
        }
