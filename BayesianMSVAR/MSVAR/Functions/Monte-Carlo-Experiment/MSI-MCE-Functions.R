...
MSI.MC.Experiment <- function(K, p, M, TT, N=1000, min.regime.occurrence, A, Sigma, PR_TR)   {
    ...
    Gen.Sigma	    <- array(NA, c(K,K,1,N))
    Gen.Y		    <- array(NA, c(TT,K,N))

	Est.Sigma	    <- array(NA, c(K,K,1,N))
    Est.A	        <- array(NA, c((M+K*p),K,N))
	...
	cat("\n______________________________________________________________________________\nMonte Carlo experiment, MSI-VAR K:", K, " p:", p, "   M:", M, "   TT:", TT, "\n______________________________________________________________________________\n") 
...
# BEGIN LOOP
        ...
        out.ser.gen     <- MSI.Series.Generation(K, p, M, TT, min.regime.occurrence, max.iter.hist,  burnin=100, A, PR_TR, Sigma)
        ...
        Gen.Sigma[,,,iterations] <- tmp.Gen.Sigma
        ...
        out.EM.wrapper  <- MSI.EM.wrapper(tmp.Gen.Y,M,p,tmp.Gen.Xi)
...
#
MSI.EM.wrapper  <- function(Y,M,p,Gen.Xi)   {

    Est.output  <- try(EM.MSI(Y, M, p, max.iter=1000, convergence=0.00001, diagonal.PR_TR=0.9, print=FALSE, plot=FALSE))
    ...
    Est.Sigma   <- array(NA, c(K,K))
    Est.A	    <- array(NA, c((M+K*p),K))
