...
G.MHM.MSVAR.reducedRankP <- function(Gibbs.output, priors, restrictions, rankP, alpha, debug=FALSE, print.iterations=100){
    ...
    # Priors
    log.prior.PR_TR <- array(NA, c(rankP,N))
...
#---------------------------------------------------------------------------------------------------
    output.matrix       <- MSVAR.posteriors.matrix.reducedRankP(Gibbs.output, restrictions, rankP)
...
#---------------------------------------------------------------------------------------------------
    ...
    for(iteration in 1:N){
        ...
        aux$PR_TR       = matrix(posteriors$PR_TR[,iteration],ncol=M)
        PR.forecasted   = G.BHLK.forecasting(aux)
        
        # Log likelihood 
        log.likelihood[iteration]   <- sum( log(colSums(eta.t * PR.forecasted)))
    }                                                                                    
...
#         } 
        PR_TR           = matrix(posteriors$PR_TR[,iteration], nrow=M)[1:rankP,]
        if (!is.matrix(PR_TR)){ PR_TR = matrix(PR_TR,nrow=1)}
        dirichlet.par   = matrix(priors$w,ncol=M)
        for (j in 1:rankP){
           log.prior.PR_TR[j,iteration]    <- log(ddirichlet(x=PR_TR[j,], alpha=matrix(priors$w,ncol=M)[j,]))
        }
    }
...
#             par.sigma   <- priors$V.Beta.bar.m1
    ...
    priors.joint        <- log.prior.Beta.joint + log.prior.R.joint + log.prior.S.joint + log.prior.PR_TR.joint
    ...
    output  <- list(    MHM                  = MHM,
                        ...
                        alpha                = alpha,
                        log.likelihood       = log.likelihood,
                        log.prior            = priors.joint,
                        log.truncated.normal = log.truncated.normal,
                        posterior.draws      = posteriors.vector)
...
MSVAR.posteriors.matrix.reducedRankP <- function(Gibbs.output, restrictions, rankP)
{
    #----------------------------------------------------------------------------- 
    # Setup constants 
    #-----------------------------------------------------------------------------
...
#         iteration.vector    <- as.vector(w.minus.last)
       ...
       PR_TR.unique     = as.vector(matrix(posteriors$PR_TR[,iteration], nrow=M)[1:rankP,1:(M-1)])
       iteration.vector = PR_TR.unique
