...
# ii) restrict A_{12.1}^{(l)}=0 for l=1,...,p
restrictions$Beta.St[c(3,5,7),1,1] = 0
...
Burnin.output <- G.MSBVAR.griddy.A3.rank1(N=N.burnin, h=h, priors=priors, restrictions=restrictions, starting.values=starting.values, debug=FALSE, print.iterations=1000)

# Gibbs algorithm: Second run
N.griddy <- 100000
t0 = proc.time()
Griddy.output <- G.MSBVAR.griddy.A3.rank1(N=N.griddy, h=h, priors=priors, restrictions=restrictions, starting.values=Burnin.output$last.draws, debug=FALSE, print.iterations=1000)
t1 <- proc.time()
time.exe <- (t1-t0)/3600
...
# MHM
#-----------------------------------------------------------------------------------------------
MHM <- G.MHM.MSVAR.reducedRankP(Griddy.output, priors, restrictions, rankP=1, alpha, debug=FALSE, print.iterations=250)
...
# N.griddy <- 200000
# Griddy.output.f <- G.MSBVAR.griddy.A3.rank1(N=N.griddy, h=h, priors=priors, restrictions=restrictions, starting.values=Burnin.output$last.draws, debug=FALSE, print.iterations=1000)
# MHM <- G.MHM.MSVAR.reducedRankP(Griddy.output, priors, restrictions, rankP=1, alpha, debug=FALSE, print.iterations=250)
